...
Lemma mem_lfappend : forall (n : natural) (x y : lst), lst_mem n x = true -> lst_mem n (lfappend y x) = true.
Proof.
  intros.
  induction y.
...
Lemma subset_lfappend_general : forall (x y z : lst), lst_subset x y = true -> lst_subset x (lfappend z y) = true.
Proof.
  intros.
  induction x.
...
Proof.
  ...
  apply Bool.andb_true_iff in H. destruct H. unfold lst_eq. apply Bool.andb_true_iff. split.
    + simpl. apply Bool.andb_true_iff. split.
      * 
      rewrite lfappend_single. apply subset_lfappend_general. assumption.
      * 
