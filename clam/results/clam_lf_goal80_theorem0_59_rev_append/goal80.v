...
Theorem theorem0 : forall (x : lst) (y : lst), eq (rev (lfappend (rev x) (rev y))) (lfappend y x).
Proof.
   induction x.
   - intros. simpl. rewrite rev_rev. rewrite lfappend_nil. reflexivity.
   - intros.  simpl.  
   lfind_coqsynth.  simpl.  rewrite (eq_refl : Cons n Nil = rev (Cons n Nil)).  rewrite IHx.  rewrite rev_rev.  simpl.  reflexivity. 
Admitted.
