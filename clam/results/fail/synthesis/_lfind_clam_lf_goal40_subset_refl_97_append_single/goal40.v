...
:= match mem_arg0, mem_arg1 with
  | x, Nil => false
  | x, Cons y z => orb (Neqb x y) (mem z x)
  end.
...
Lemma Nat_beq_refl : forall (n : natural), Neqb n n = true.
Proof.
  intros.
  induction n.
  - assumption.
...
Lemma append_cons : forall (x y : lst) (n : natural), lfappend x (Cons n y) = lfappend (lfappend x (Cons n Nil)) y.
Proof.
  intros.
  induction x.
...
Lemma subset_append : forall (x y : lst), lst_subset x (lfappend y x) = true.
Proof.
  intro.
  induction x.
...
Proof.
  ...
  - simpl. apply Bool.andb_true_iff. split.
   + 
 lfind_coqsynth.  apply subset_append. 
Admitted.
