...
Proof.
   intro.
   induction x.
   - intros. simpl. rewrite IHx. rewrite (IHx (Cons n Nil)). rewrite lfappend_assoc. reflexivity.
   - reflexivity.
   
Qed.

Lemma qreva_lfappend : forall (x y : lst), qreva (lfappend x y) Nil = lfappend (qreva y Nil) (qreva x Nil).
Proof.
   intros.
   induction x.
   - simpl. rewrite qreva_nil. rewrite IHx. rewrite lfappend_assoc. rewrite (qreva_nil x (Cons n Nil)). reflexivity.
...
Proof.
   ...
   induction x.
   - simpl.  
   lfind_coqsynth.  rewrite qreva_lfappend.  rewrite IHx.  simpl.  reflexivity. 
Admitted.
