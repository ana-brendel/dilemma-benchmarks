...
Fixpoint lfeqb (n m: natural) : bool :=
  ...
  := match mem_arg0, mem_arg1 with
    | x, Nil => false
    | x, Cons y z => orb (lfeqb x y) (mem z x)
    end.
...
Lemma Nat_beq_refl : forall (n : natural), lfeqb n n = true.
Proof.
  intros.
  induction n.
  - auto.
...
Lemma append_single : forall (x : lst) (n : natural), Cons n x = lfappend (Cons n Nil) x.
Proof.
  reflexivity.
Qed.

Lemma mem_append : forall (n : natural) (x y : lst), lst_mem n x = true -> lst_mem n (lfappend y x) = true.
Proof.
  intros.
  induction y.
...
Lemma subset_append_general : forall (x y z : lst), lst_subset x y = true -> lst_subset x (lfappend z y) = true.
Proof.
  intros.
  induction x.
...
Proof.
  ...
  apply Bool.andb_true_iff in H. destruct H. unfold lst_eq. apply Bool.andb_true_iff. split.
    ...
    + simpl. apply Bool.andb_true_iff. split.
      * 
 lfind_coqsynth.  apply subset_append_general.  assumption. 
Admitted.
