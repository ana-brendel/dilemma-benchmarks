...
Proof.
   ...
   induction x.
   - reflexivity.
   - simpl. rewrite rev_lfappend. simpl. rewrite IHx. reflexivity.
Qed.

Theorem theorem0 : forall (x : lst) (y : lst), eq (rev (qreva x (rev y))) (lfappend y x).
Proof.
   induction x.
   - intros. simpl. rewrite rev_rev. rewrite lfappend_nil. reflexivity.
   - intros.  simpl.  rewrite (eq_refl : Cons n (rev y) = lfappend (rev (Cons n Nil)) (rev y)).  
   lfind_coqsynth.  rewrite IHx.  rewrite lfappend_assoc.  simpl.  reflexivity. 
Admitted.
