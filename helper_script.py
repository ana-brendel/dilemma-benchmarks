...
goals = {"goal1.v": [["theorem0", 31, "lem"]],"goal2.v": [["theorem0", 40, "lem"], ["theorem0", 41, "lem2"]],"goal4.v": [["theorem0", 39, "lem"]],"goal5.v": [["theorem0", 46, "lem"], ["theorem0", 47, "lem2"]],
...
for name in goals.keys():
    n = name.removesuffix(".v")
    locations = goals[name]
    for l in locations:
        a = l[0]
        b = l[1]
        c = l[2]
        test = f"clam_lf_{n}_{a}_{b}_{c}"
        benches.append(test)

total = 0
for f in os.listdir("/home/anabrendel/lfind/benchmarks/clam/sources"):
    if f not in benches:
        # print(f)
        total += 1

for f in benches:
    if f not in os.listdir("/home/anabrendel/lfind/benchmarks/clam/sources"):
        print(f)

print(len(benches))
print(len(os.listdir("/home/anabrendel/lfind/benchmarks/clam/sources")))
print(total)
print(total + len(benches))
