...
Fixpoint domain_equal (A B : Type) (m0 : Map A) (m1 : Map B) {struct m1} :
 Prop :=
  match m0, m1 with
  | M0, M0 => True
  | M0, M1 _ _ => False
...
Lemma domain_equal_mapget :
 forall (A B : Type) (m0 : Map A) (m1 : Map B) (a : ad) (x : A),
...
Lemma domain_equal_reflexive :
 forall (A : Type) (m : Map A), domain_equal A A m m.
Proof.
...
Lemma domain_equal_symmetric :
 forall (A B : Type) (m0 : Map A) (m1 : Map B),
...
Lemma domain_equal_transitive :
 forall (A0 A1 A2 : Type) (m0 : Map A0) (m1 : Map A1) (m2 : Map A2),
...
Definition mEnsemble (A : Type) := Map A -> Prop.

Definition mRelation (A : Type) := Map A -> Map A -> Prop.

Definition r_symmetric (A : Type) (r : mRelation A) :=
  forall x y : Map A, r x y -> r y x.

Definition r_antisymmetric (A : Type) (r : mRelation A) :=
  forall x y : Map A, r x y -> r y x -> x = y.

Definition r_transitive (A : Type) (r : mRelation A) :=
  forall x y z : Map A, r x y -> r y z -> r x z.

Definition r_reflexive (A : Type) (r : mRelation A) := forall x : Map A, r x x.

Definition r_order (A : Type) (r : mRelation A) :=
  r_reflexive A r /\ r_antisymmetric A r /\ r_transitive A r.

Definition mini (A : Type) (r : mRelation A) (T : mEnsemble A) 
  (e : Map A) := T e /\ (forall x : Map A, T x -> r e x).

Definition maxi (A : Type) (r : mRelation A) (T : mEnsemble A) 
  (e : Map A) := T e /\ (forall x : Map A, T x -> r x e).

Definition mLattice (A : Type) (r : mRelation A) (T : mEnsemble A)
  (e f : Map A) := r_order A r /\ mini A r T e /\ maxi A r T f.

Inductive prechain (A : Type) : Type :=
  | single : Map A -> prechain A
  | concat : prechain A -> Map A -> prechain A.

Lemma prechain_sum :
 forall (A : Type) (p : prechain A),
...
Inductive prechain_dom_ok (A : Type) : mEnsemble A -> prechain A -> Prop :=
  | domok_single :
...
Fixpoint chain_length (A : Type) (p : prechain A) {struct p} : nat :=
  match p with
  | single x => 1
  | concat x y => S (chain_length A x)
  end.

Definition prechain_last (A : Type) (p : prechain A) : 
  Map A := match p with
           | single x => x
           | concat z x => x
           end.

Inductive prechain_incr (A : Type) : mRelation A -> prechain A -> Prop :=
  | incr_single :
...
Lemma chain_def_ok :
 forall (A : Type) (T : mEnsemble A) (r : mRelation A) (p : prechain A),
...
Lemma pre_domok_incr_chain_2 :
 forall (A : Type) (p : prechain A), pre_domok_incr_chain_def A p.
Proof.
	intro. exact
...
Lemma pre_domok_incr_chain :
 forall (A : Type) (p : prechain A) (T : mEnsemble A) (r : mRelation A),
 prechain_dom_ok A T p /\ prechain_incr A r p -> chain A T r p.
Proof.
	intro. exact
  (prechain_ind A (pre_domok_incr_chain_def A) (pre_domok_incr_chain_0 A)
...
Inductive dist_chain (A : Type) : prechain A -> Prop :=
...
Inductive non_dist_chain (A : Type) : prechain A -> Prop :=
  | non_dist_concat_s :
...
Definition sas_chain (A : Type) (T : mEnsemble A) (r : mRelation A)
  (p : prechain A) : Prop := chain A T r p /\ dist_chain A p.

Definition dist_compl_def_0 (A : Type) (p : prechain A) : Prop :=
  dist_chain A p \/ non_dist_chain A p.

Lemma dist_compl_0 :
 forall (A : Type) (m : Map A), dist_compl_def_0 A (single A m).
Proof.
	unfold dist_compl_def_0 in |- *. intros. left. exact (dist_single A m).
Qed.

Definition dist_compl_def_1 (A : Type) (p : prechain A) : Prop :=
  dist_compl_def_0 A p -> forall m : Map A, dist_compl_def_0 A (concat A p m).

Lemma dist_compl_1 :
 forall (A : Type) (m : Map A), dist_compl_def_1 A (single A m).
Proof.
...
Lemma dist_compl_4 :
 forall (A : Type) (p : prechain A), dist_chain A p \/ non_dist_chain A p.
Proof.
	intro. exact
...
Lemma dist_compl_5 :
 forall (A : Type) (x : Map A), ~ dist_chain A (concat A (single A x) x).
Proof.
	intros. intro. inversion H. exact (H1 (refl_equal x)).
Qed.

Lemma dist_compl_6 :
 forall (A : Type) (x : Map A) (z : prechain A),
...
Lemma dist_compl_7 :
 forall (A : Type) (x y : Map A) (z : prechain A),
 non_dist_chain A (concat A z x) ->
 ~ dist_chain A (concat A z x) -> ~ dist_chain A (concat A (concat A z x) y).
Proof.
...
Lemma dist_compl_8 :
 forall (A : Type) (p : prechain A), non_dist_chain A p -> ~ dist_chain A p.
Proof.
	exact
  (fun A : Type =>
...
Definition bounded_sas_chain (A : Type) (T : mEnsemble A) 
  (r : mRelation A) (n : nat) : Prop :=
  forall p : prechain A, sas_chain A T r p -> chain_length A p <= n.

Definition def_ok_app (A : Type) (T : mEnsemble A) (f : Map A -> Map A) :
  Prop := forall x : Map A, T x -> T (f x).

Definition increasing_app (A : Type) (r : mRelation A) 
  (f : Map A -> Map A) : Prop := forall x y : Map A, r x y -> r (f x) (f y).

Definition fix_point (A : Type) (T : mEnsemble A) (f : Map A -> Map A)
  (x : Map A) : Prop := T x /\ f x = x.

Definition inf_fix_points (A : Type) (T : mEnsemble A) 
  (r : mRelation A) (f : Map A -> Map A) (x : Map A) : Prop :=
  forall y : Map A, fix_point A T f y -> r x y.

Definition lower_fix_point (A : Type) (T : mEnsemble A) 
  (r : mRelation A) (f : Map A -> Map A) (x : Map A) : Prop :=
  fix_point A T f x /\ inf_fix_points A T r f x.

(* itération d'une fonction *)

Fixpoint iteres (A : Type) (f : Map A -> Map A) (x : Map A) 
...
Fixpoint power (A : Type) (f : A -> A) (x : A) (n : nat) {struct n} : A :=
  match n with
  | O => x
  | S n => f (power A f x n)
  end.

Inductive MapFlag (A : Type) : Type :=
  | flag_true : Map A -> MapFlag A
  | flag_false : Map A -> MapFlag A.

Lemma MapFlag_sum :
 forall (A : Type) (f : MapFlag A),
...
Fixpoint lazy_power_aux (A : Type) (egalite : Map A -> Map A -> bool)
...
Definition lazy_power (A : Type) (egalite : Map A -> Map A -> bool)
...
Lemma lazy_power_eg_power_0 :
 forall (A : Type) (egalite : Map A -> Map A -> bool) 
...
Fixpoint iteres_0 (A : Type) (f : Map A -> Map A) (x : Map A) 
 (n : nat) {struct n} : prechain A :=
  match n with
  | O => single A x
  | S p =>
...
Lemma iteres_eq_0 :
 forall (A : Type) (f : Map A -> Map A) (x : Map A) (n : nat),
...
Lemma iteres_def_ok :
 forall (A : Type) (T : mEnsemble A) (f : Map A -> Map A) 
   (x : Map A) (n k : nat),
...
Definition iteres_ult_const_def_0 (A : Type) (p : prechain A) : Prop :=
...
Lemma iteres_last :
 forall (A : Type) (f : Map A -> Map A) (x : Map A) 
...
Lemma iteres_incr :
 forall (A : Type) (r : mRelation A) (f : Map A -> Map A) 
   (x : Map A) (n : nat),
...
Lemma iteres_increasing_chain :
 forall (A : Type) (T : mEnsemble A) (r : mRelation A) 
   (f : Map A -> Map A) (x : Map A) (n : nat),
 T x ->
 def_ok_app A T f ->
...
Definition ensemble_base (A : Type) (m : Map A) (x : Map bool) :=
  domain_equal A bool m x.

Fixpoint map_fill (A : Type) (m : Map A) {struct m} : 
 bool -> Map bool :=
  fun b : bool =>
  match m with
  | M0 => M0 bool
...
Definition map_mini (A : Type) (m : Map A) : Map bool := map_fill A m false.

Definition map_maxi (A : Type) (m : Map A) : Map bool := map_fill A m true.

Lemma map_mini_appartient :
 forall (A : Type) (x : Map A), ensemble_base A x (map_mini A x).
Proof.
...
Lemma map_maxi_appartient :
 forall (A : Type) (x : Map A), ensemble_base A x (map_maxi A x).
Proof.
	unfold ensemble_base in |- *. intros. induction  x as [| a a0| x1 Hrecx1 x0 Hrecx0]. simpl in |- *. exact I. simpl in |- *.
	reflexivity. simpl in |- *. split. exact Hrecx1. exact Hrecx0.
Qed.
...
Lemma map_mini_mapget_false :
 forall (A : Type) (x : Map A) (a : ad) (b : bool),
...
Lemma lattice_bounded_5 :
 forall (p : prechain bool) (A : Type) (m0 m1 : Map A),
 sas_chain bool (ensemble_base A (M2 A m0 m1)) lem p ->
 exists p0 : prechain bool,
   (exists p1 : prechain bool,
      sas_chain bool (ensemble_base A m0) lem p0 /\
...
Definition lattice_bounded_def_2 (A : Type) (m : Map A) : Prop :=
  forall p : prechain bool,
  sas_chain bool (ensemble_base A m) lem p ->
  chain_length bool p <= S (MapCard A m).

Lemma lattice_bounded_6 : forall A : Type, lattice_bounded_def_2 A (M0 A).
Proof.
...
Lemma lattice_bounded_7 :
 forall (A : Type) (a : ad) (a0 : A), lattice_bounded_def_2 A (M1 A a a0).
Proof.
...
Lemma lattice_bounded_9 :
 forall (A : Type) (m : Map A), lattice_bounded_def_2 A m.
Proof.
	intro. exact
...
Lemma lattice_bounded_10 :
 forall (A : Type) (m : Map A) (p : prechain bool),
 sas_chain bool (ensemble_base A m) lem p ->
 chain_length bool p <= S (MapCard A m).
Proof.
	intro. exact
