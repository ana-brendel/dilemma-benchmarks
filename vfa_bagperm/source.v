...
Proof.
  ...
  assert (0 = (if a =? a then 1 else 0) + count a b). apply (H a). rewrite <- HeqH1 in H0. lia. symmetry in HeqH1. apply Nat.eqb_neq in HeqH1. lia.
Qed.
...
Proof.
  ...
  assert (S n = count x l). lia. apply IHl in H1. inversion H1 as [l1 [l2 [H2 H3]]]. exists (a :: l1), l2. split. simpl. rewrite H2. reflexivity. rewrite H0. simpl. bdestruct (x =? x). lia. lia. simpl in H. apply IHl in H. inversion H as [l1 [l2 [H1 H2]]]. exists (a :: l1), l2. split. simpl. rewrite H1. reflexivity. simpl. (bdestruct (a =? x)). lia. lia.
Qed.
...
Proof.
  ...
  bdestruct (a =? n). rewrite H0. constructor. apply IHal. rewrite H0 in H. apply (bag_eqv_uncons n). assumption.
  unfold bag_eqv in H. assert (count a (a :: al) = count a (n :: bl)). apply H. simpl in H1. bdestruct (a =? a). bdestruct (n =? a). lia. simpl in H1.
  ...
  bdestruct (n0 =? a). rewrite H8. simpl. bdestruct (n =? a). lia. simpl. rewrite H7. reflexivity. rewrite H6 in H. assert (count n0 (a :: al) = count n0 (n :: x ++ a :: x0)). apply H. rewrite app_comm_cons.
  rewrite <- (count_insert_other (n :: x) x0 a n0). rewrite <- H.
  simpl. bdestruct (a =? n0). lia. reflexivity.  
  assumption. apply (perm_skip a) in H8. rewrite app_comm_cons in H8. rewrite Permutation_middle in H8. simpl in H8. rewrite <- H6 in H8. assumption. lia.
Qed.
