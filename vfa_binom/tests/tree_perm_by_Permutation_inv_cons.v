...
Proof.
  ...
  assert (pow2heap (S n) (smash (Node n1 t1 t2) (Node n0 a1 a2))). 
  (* HELPER LEMMA $ carry_valid_by_smash_valid $ *)
  apply smash_valid. assumption. inversion H. inversion H2. contradict H4. discriminate. 
  assumption. inversion H. unfold priqq. split. left. reflexivity. fold priqq. apply IHq. assumption. right. assumption.
...
Theorem insert_priq: forall x q, priq q -> priq (insert x q).
  ...
  ++ unfold priq, insert in *.
    simpl in H.
    inversion H.
    (* HELPER LEMMA $ smash_valid_by_carry_valid_1 $ *)
    apply carry_valid.
    simpl.
    split; auto.
    right. simpl. auto.
  ++ unfold priq, insert in *.
    (* HELPER LEMMA $ smash_valid_by_carry_valid_2 $ *)
    apply carry_valid.
    - auto.
    - right. simpl. auto.
Qed.
...
Lemma Permutation_inv_cons: forall (l l' : list nat) (a: nat),
    Permutation l l' -> Permutation (a :: l) (a :: l').
Proof.
    intros.
    (* HELPER LEMMA $ Permutation_inv_cons_by_Permutation_trans_1 $ *)
    apply Permutation_trans with (l' := (l ++ [a])).
    - (* HELPER LEMMA $ Permutation_inv_cons_by_Permutation_cons_append_1 $ *)
    apply Permutation_cons_append.
    - 
    (* HELPER LEMMA $ Permutation_inv_cons_by_Permutation_trans_2 $ *)
    apply Permutation_trans with (l' := (l' ++ [a])).
    -- (* HELPER LEMMA $ Permutation_inv_cons_by_app_tail $ *)
    apply Permutation_app_tail. assumption.
    -- symmetry.
    (* HELPER LEMMA $ Permutation_inv_cons_by_Permutation_cons_append_2 $ *)
    apply Permutation_cons_append.
Qed.
...
Proof.
  ...
  (* HELPER LEMMA $ tree_perm_by_Permutation_inv_cons $ *)
findlemma. Admitted.
(*
  apply Permutation_inv_cons.
  (* HELPER LEMMA $ tree_perm_by_Permutation_apps $ *)
