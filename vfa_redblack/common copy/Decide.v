...
(* ************************** [ FORALLT ] ************************** *)
(* **************************************************************** *)
Instance ForallT_dec (P: nat -> nat -> Prop) `{forall (k : nat) (x : nat), Dec (P k x)} (t: tree) : (Dec (ForallT P t)).
Proof. 
  dec_eq. induction t.
  - left. simpl. auto.
  - simpl.
  + assert (Q: {P n n0} + {~ P n n0}). apply H. destruct Q.
...
Proof. 
  dec_eq. induction t.
  - left. apply BST_E.
  - assert (Q1: {ForallT (fun (y : nat) (_ : nat) => y < n) t1} + {~ ForallT (fun (y : nat) (_ : nat) => y < n) t1}).
  apply ForallT_dec. intros. dec_eq. apply lt_dec. 
  assert (Q2: {ForallT (fun (y : nat) (_ : nat) => n < y) t2} + {~ ForallT (fun (y : nat) (_ : nat) => n < y) t2}).
  apply ForallT_dec. intros. dec_eq. apply lt_dec.
  destruct IHt1. 
...
Proof.
  ...
  -- destruct n1. simpl in H. discriminate. simpl in H. apply andb_prop in H. destruct H.
