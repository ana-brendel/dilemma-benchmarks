...
From vfa_searchtree_benchmarks Require Import Definitions.
...
Proof. 
  ...
  ++ destruct H0. unfold disjoint in H2. apply H2. assumption.
  + intros. unfold disjoint in H0.   
  assert (Q: {In a (a :: l1)} + {~ In a (a :: l1)}). apply In_dec. intros. apply H.
  destruct Q. split. apply H0. assumption. unfold disjoint. intros. apply H0. simpl.
  right. assumption. 
  simpl in n. assert (P: a = a <-> True). split. reflexivity. reflexivity. rewrite P in n.
  assert (F: forall p, (True \/ p) <-> True). split. intros. auto. intros. left. auto.
  rewrite F in n. contradiction.
Qed.

Instance BST_disjoint {V} `{_ : Dec_Eq V} (l1 l2: list V) : (Dec (disjoint l1 l2)).
Proof.
  dec_eq. induction l1. 
  - unfold disjoint. simpl. left. intros. auto.
  - assert (Q: {In a l2} + {~ In a l2}). apply In_dec. intros. apply H.
  destruct Q. right. rewrite <- disjoint_cons. unfold not. intros.
