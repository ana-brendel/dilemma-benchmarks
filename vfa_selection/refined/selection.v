...
Proof. intros l x r HL y r' Hsel. apply select_rest_length in Hsel. inversion Hsel. simpl. lia. Defined.

Lemma selsortt_perm : forall n l, length l = n -> Permutation l (selsort' l).
Proof. 
    induction n.
    - intros. destruct l. auto. inversion H.
    - intros. destruct l. 
...
Proof. 
    intros. induction (selsort' bl) eqn:K.
    - apply sorted_1.
    - apply sorted_cons.
    -- eapply le_all__le_one. eauto. 
    eapply Permutation_in.
...
Lemma selsortt_sorted : forall n al, length al = n -> sorted (selsort' al).
Proof. 
    intros n. induction n.
    - intros. simpl. rewrite selsort'_equation. destruct al. apply sorted_nil. simpl in H. lia.
    - intros. destruct al.
    -- simpl in H. lia.
    -- simpl. rewrite selsort'_equation. destruct (select n0 al) eqn:Q. apply cons_of_small_maintains_sortt.
    --- eapply select_smallest. eauto.
    --- apply IHn. apply select_rest_length in Q. rewrite <- Q. simpl in H. lia.
Qed.

Theorem selsortt_is_correct : is_a_sorting_algorithm selsort'.
Proof. 
    unfold is_a_sorting_algorithm. split. 
    eapply selsortt_perm. exists.
    eapply selsortt_sorted. exists.
Qed.